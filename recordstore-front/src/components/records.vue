<template>
  <div class="max-w-md m-auto py-10">
    ...
    <form @submit.prevent="addRecord">
      ...
      <input type="submit" value="Add Record" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green-600 hover:bg-green-600-dark block w-full py-4 text-white items-center justify-center">
    </form>
    ...
    <ul class="list-reset mt-4">
      <li class="py-4" v-for="record in records" :key="record.id" :record="record">
        ...
        <button class="bg-transparent text-sm hover:bg-blue-600 hover:text-white text-blue border border-blue no-underline font-bold py-2 px-4 mr-2 rounded"
        ...
        <div v-if="record == editedRecord">
          <form action="" @submit.prevent="updateRecord(record)">
            <div class="mb-6 p-4 bg-white rounded border border-grey-light mt-4">
              ...
              <input type="submit" value="Update" class="bg-transparent text-sm hover:bg-blue-600 hover:text-white text-blue border border-blue no-underline font-bold py-2 px-4 mr-2 rounded">
            </div>
          </form>
        </div>
      </li>
